...
function Experience({ author, text, id }: IExperience) {
  return (
    <div
      className="experience not-dark:bg-neutral-200 dark:bg-neutral-700"
      key={id}
    >
      <p>{text}</p>
      <span>{author}</span>
    ...
    <section className="experiences">
      ...
      <div className="cards">
        {width > 768 ? (
          ...
          <div>
            {experiences.map(({ author, text, id }) => (
              <Experience id={id} author={author} text={text} />
            ))}
          </div>
        )}
      </div>
