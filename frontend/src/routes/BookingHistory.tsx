  ...
  const handleRate = async (bookingId: number, rating: number) => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/v1/rate/${bookingId}`,
        {
          method: 'PUT',
          credentials: 'include',
          headers: {
          ...
          {bookings.map((booking) => (
            <div key={booking.id} className="booking-card">
              ...
              <div className="booking-details">
                <p>
                  <strong>Hotel:</strong>{' '}
                  {hotels[booking.hotel_id]?.name || 'Loading...'}
                </p>
                <p>
                ...
                {booking.rating === 0 && (
                  <div className="rating-component">
                    <select
                      defaultValue={0}
                      onChange={(e) =>
                        handleRate(booking.id, Number(e.target.value))
                      }
                    >
            ...
            {hotels[selectedBookingForInvoice.hotel_id] ? (
              <>
                <div className="invoice-details">
                  <p>
                    <strong>Hotel:</strong>{' '}
                    {hotels[selectedBookingForInvoice.hotel_id].name}
                  </p>
                  <p>
                    <strong>Check-in:</strong>{' '}
                    {new Date(
                      selectedBookingForInvoice.checkin,
                    ).toLocaleDateString()}
                  </p>
                  <p>
                    <strong>Check-out:</strong>{' '}
                    {new Date(
                      selectedBookingForInvoice.checkout,
                    ).toLocaleDateString()}
                  </p>
                  ...
                  {(() => {
                    ...
                    const pricePerDay =
                      hotels[selectedBookingForInvoice.hotel_id].price
                    ...
                    return (
                      <>
                        <p>
                          <strong>Days Stayed:</strong> {diffDays}
                        </p>
                        <p>
                        ...
                        <hr />
                        <p>
                          <strong>Before Tax:</strong> ${beforeTax.toFixed(2)}
                        </p>
                        <p>
                          <strong>Tax (27%):</strong> ${tax.toFixed(2)}
                        </p>
                        <p className="total">
